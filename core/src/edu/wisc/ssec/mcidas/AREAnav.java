    ...
    implements java.io.Serializable
{
    ...
    public final static double RADIANS_TO_DEGREES = 180.0 / PI;

    /** Constant for degrees to radians conversion */
    public final static double DEGREES_TO_RADIANS = PI / 180.0;
    ...
    /** {@literal "Line"} index in line/element array */
    public final int indexLine = 1;
    
    /** {@literal "Element"} index in line/element array */
    public final int indexEle = 0;
    
    /** {@literal "Latitude"} index in latitude/longitude array */
    public final int indexLat = 0;
    
    /** {@literal "Longitude"} index in latitude/longitude array */
     ...
     * was blown up when displayed).  This value is always {@code > 0}.
     *
    ...
    */
                                            ...
                                            double[][] newvals)
    {
        ...
        for (int i = 0; i < linele[0].length; i++)
        {
            if (linele[indexLine][i] == linele[indexLine][i]) {
                ...
                // account for flipped coordinates
                if (isLineFlipped) {
                    newvals[indexLine][i] = lineOffset - newvals[indexLine][i];
                }
            }
            if (linele[indexEle][i] == linele[indexEle][i]) {
    ...
    /**
     * Determines whether or not the {@code Object} in question is
     * the same as this {@code AREAnav}.
     *
    ...
    @Override public boolean equals(Object obj) {
        // return false;
        // WLH 13 April 2000, this broke visad.data.mcidas.TestArea
    ...
    /**
     * Return a {@code String} representation of this nav module.
     *
     * @return Wordy string.
     */
    ...
    public static double[] solarp(int jday, int jtime) {
        ...
        final double oincli = DEGREES_TO_RADIANS * mcPackedIntegerToDouble(232700);
        final double perhel = DEGREES_TO_RADIANS * mcPackedIntegerToDouble(1011311) + PI;
        final double asnode = DEGREES_TO_RADIANS * mcPackedIntegerToDouble(0);
...
//        Dummy.label("solarp/Solarp",3);
        ...
        final double slra = atan2(ys, xs) / DEGREES_TO_RADIANS;
        ...
        final double dec = atan2(zs, sqrt(pow(xs, 2) + pow(ys, 2))) / DEGREES_TO_RADIANS;
        final double xlat = geolat(dec * DEGREES_TO_RADIANS, 1) / DEGREES_TO_RADIANS;
